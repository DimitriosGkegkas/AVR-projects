...
uint16_t get_temp(void){
	...
	uint16_t a= 0x0000+ds18b20_readbyte();
	uint16_t b= 0x0000 + ds18b20_readbyte();
	a=a << 8;
	uint16_t re= a+b;
	return re;
}
...
int main(){
	while(1){
		...
		if(temp=0x8000){
			LCD_String ("No Dev-Pad Active");	
			while(1){
				char a1=keyfind(); 							//reading the 4 digits
				char a2=keyfind();
				char a=keyfind();
				char b=keyfind();
				LCD_Init();
				if(a1=='8' && a2=='0'&& a=='0'&& b=='0' ){
					LCD_Init();
					LCD_String ("No Device");	
					continue;
				}

				if(a>0x3A){	a=0x0F & a;a=a+9;}				//genereting there number from the ascii form.
				if(b>0x3A){b=0x0F & b;b=b+9;}
				b=0x0F & b;a=0x0F & a;
				a=a<<4;
				char res=a+b;								//Now res has the value that we have to display
				if(res>=0x80) {					
					LCD_Char('-');
					res=res-1;
					res= ~res;
